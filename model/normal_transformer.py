...
class EncoderLayer(nn.Module):
    ...
    def forward(self, enc_input, slf_attn_mask=None):
        enc_output, enc_slf_atn = self.slf_attn(
            enc_input, enc_input, enc_input, mask=slf_attn_mask)
        enc_output = self.pos_ff(enc_output)
        return enc_output, enc_slf_atn
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, dec_input, enc_output,
        ...
        dec_output, enc_enc_attn = self.enc_attn(
            dec_output, enc_output, enc_output, mask=dec_enc_attn_mask)

        dec_output = self.pos_ff(dec_output)
        return dec_output, dec_slf_attn, dec_enc_attn_mask
...
class Decoder(nn.Module):
    ...
    def forward(self, trg_seq, trg_mask, enc_output, src_mask,
                return_attns=False):
        dec_slf_attn_list, enc_slf_attn_list = [], []

        dec_output = self.trg_world_emb(trg_seq)
        if self.scale_embed:
        ...
        for dec_layer in self.layer_stack:
            ...
            enc_slf_attn_list += [dec_enc_attn] if return_attns else []

        if return_attns:
            return dec_output, dec_slf_attn_list, enc_slf_attn_list
        return dec_output,
...
class Transformer(nn.Module):
    def __init__(self, n_src_vocab, n_trg_vocab, src_pad_idx, trg_pad_idx,
        ...
        # In section 3.4 of paper "Attention Is All You Need":
        ...
        scale_emb = (scale_emb_or_prj == 'emb') \
            if trg_emb_prj_weight_sharing else False
        ...
        self.encoder = Encoder(
            ...
            pad_idx=src_pad_idx, dropout=dropout, scale_embed=scale_emb)
        ...
        if trg_emb_prj_weight_sharing:
            self.trg_word_prj.weight = self.decoder.trg_world_emb.weight

        if emb_src_trg_weight_sharing:
            self.encoder.src_word_emb.weight = \
                self.decoder.trg_world_emb.weight
